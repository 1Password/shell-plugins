...
func TestSTSProvisionerErrorsOnSSOSourceProfile(t *testing.T) {
	t.Setenv("AWS_PROFILE", "")
	t.Setenv("AWS_DEFAULT_REGION", "")
	configPath := filepath.Join(t.TempDir(), "awsConfig")
	t.Setenv("AWS_CONFIG_FILE", configPath)
	...
	ssoSource, err := file.NewSection("profile sso-src")
	require.NoError(t, err)
	_, err = ssoSource.NewKey("sso_start_url", "https://example.awsapps.com/start")
	require.NoError(t, err)
	_, err = ssoSource.NewKey("sso_region", "us-east-1")
	require.NoError(t, err)
	_, err = ssoSource.NewKey("sso_account_id", "111111111111")
	require.NoError(t, err)
	_, err = ssoSource.NewKey("sso_role_name", "ReadOnly")
	require.NoError(t, err)

	chained, err := file.NewSection("profile chained")
	require.NoError(t, err)
	_, err = chained.NewKey("role_arn", "arn:aws:iam::111111111111:role/Chained")
	require.NoError(t, err)
	_, err = chained.NewKey("source_profile", "sso-src")
	require.NoError(t, err)

	err = file.SaveTo(configPath)
	require.NoError(t, err)

	// A role-chain profile whose source profile is SSO-configured is handled by neither
	// provisioner: the top-level profile is not SSO, so the SSO provisioner yields, and the
	// source profile has no long-lived keys. The STS provisioner must surface a clear error
	// rather than failing later with a misleading missing-access-key message.
	plugintest.TestProvisioner(t, STSProvisioner{
		profileName: "chained",
		newProviderFactory: func(cacheState sdk.CacheState, cacheOps sdk.CacheOperations, fields map[sdk.FieldName]string) STSProviderFactory {
			return &mockProviderManager{}
		...
		"SSO source profile in role chain errors clearly": {
			ItemFields: map[sdk.FieldName]string{
				fieldname.AccessKeyID:     "AKIAHPIZFMD5EEXAMPLE",
				fieldname.SecretAccessKey: "lBfKB7P5ScmpxDeRoFLZvhJbqNGPoV0vIEXAMPLE",
			},
			ExpectedOutput: sdk.ProvisionOutput{
				Diagnostics: sdk.Diagnostics{Errors: []sdk.Error{{Message: `profile "sso-src" is configured for AWS IAM Identity Center (SSO) and is not yet supported as the source profile of a role chain by the AWS Shell Plugin`}}},
			},
		},
	})
}
